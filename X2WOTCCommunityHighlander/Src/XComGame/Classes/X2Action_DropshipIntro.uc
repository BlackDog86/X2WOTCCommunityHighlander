...
function Init()
{
	...
	numIntros = MissionIntro.MatineeSequences.Length;
	MatineeSequenceIndex = `SYNC_RAND(numIntros);

	if (MissionIntro.bSilenceMissionBriefingAtStart)
	{
		NarrativeMgr = `PRES.m_kNarrativeUIMgr;
		NarrativeMgr.StopConversations();
	}
}
