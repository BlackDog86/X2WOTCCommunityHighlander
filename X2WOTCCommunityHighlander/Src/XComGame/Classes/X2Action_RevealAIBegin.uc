...
Begin:
	...
	if( RevealedUnitVisualizers.Length > 0 )
	{
		...
		if( !bOnlyFrameAI || (WantsToPlayTheLostCamera() && !CanPlayTheLostCamera()) )
		{
			...
			if( RevealContext.FirstSightingMoment != none && (`PRESBASE.GetTimesPlayed(RevealContext.FirstSightingMoment) < RevealContext.FirstSightingMoment.AmbientMaxPlayCount)))
			{
