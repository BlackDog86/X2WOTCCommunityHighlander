...
var protected double MatineesPreviousTickTime; // make matinees run using wall clock time ( keeps multi-minute audio tracks in sync )

var private bool bPlayFinalCinematic; //Used to play the final cinematic seamlessly after the in-game matinee

// if true, units will remain hidden after the matinee finishes playing
var PostMatineeVisibility PostMatineeUnitVisibility;

// if true, the X2Camera_Matinee this action uses will allow Proximity Dither
var protected bool bUseProximityDither;
...
native protected function PauseMatinee();
native protected function ResumeMatinee();
...
{
	if (GameStateUnit.bRemovedFromPlay || GameStateUnit.bRemoved)
	{
		return;
	}

	BindUnitToMatineeGroup(GroupName, GameStateUnit);
}

function BindUnitToMatineeGroup(name GroupName, XComGameState_Unit GameStateUnit)
{
...
function XComUnitPawn FindPawnForUnitInMatinee(StateObjectReference UnitRef)
{
	local UnitToMatineeGroupMapping UnitMapping;
	local XComUnitPawn FoundPawn;

	foreach UnitMappings(UnitMapping)
	{
	...
	for(Index = 0; Index < UnitMappings.Length; Index++)
	{
		...
		if(UnitMappings[Index].Unit != none)
		{
			...
			AnimTreeTemplate = AnimTree(DynamicLoadObject("Soldier_ANIMTREE.AT_SoldierCustom_CIN", class'AnimTree'));
 			MatineePawn.Mesh.SetAnimTreeTemplate(AnimTreeTemplate);
...
protected function PlayMatinee()
{
	...
	if( !bNewUnitSelected && MatineeBases.Length > 0 )
	{
		...
		MatineeCamera.m_bAllowProximityDither = bUseProximityDither;

		for (i = 0; i < UnitMappings.Length; ++i)
		{
			MatineeCamera.m_kUnitsInMatinee.AddItem(UnitMappings[i].CreatedPawn);
		}
	...
	//Dirty, but will ensure a smooth transition for the end game sequence.
	if (Matinees.Length == 1)
	{
		Ruleset = `TACTICALRULES;
		if (Matinees[0].ObjComment == "FortressVictory" && Ruleset.IsFinalMission())
		{
			bPlayFinalCinematic = true;
		}
	}
...
simulated protected function EndMatinee()
{
	...
	//The tactical game rule set will know to wait for this movie to complete
	if(bPlayFinalCinematic)
	{		
		`XENGINE.PlayMovie(false, "CIN_Victory.bk2", "X2_033_Victory");				
	}
}
